		...
		methods: {
			 publish(){
				...
				if (this.imageList.length!=1) {
					uni.showModal({ content: '图片不能为空', showCancel: false, });
					return;
				}
				...
				uni.uploadFile({//该上传仅为示例,可根据自己业务修改或封装,注意:统一上传可能会导致服务器压力过大
					url: 'http://www.dqstudy.work:10001/space/upSpace', //仅为示例，非真实的接口地址
					filePath: this.imageList[0],
					name: 'file',
					formData: {//后台以post方式接收
						'userId':this.uid,//自己系统中的用户id
						'reviews': this.input_content,//moment文字部分
					},
